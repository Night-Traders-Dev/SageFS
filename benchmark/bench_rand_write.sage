## bench_rand_write.sage — Random write benchmark via VFS
...
proc main():
    ...
    let dev: String = "/tmp/sagefs_bench_rw.img"
    format_image(dev)
    ...
    i = 0
    while i < blocks:
        let off: Int = ((i * 17) % blocks) * block_size
        fs.lseek(fd, off, vfs.SEEK_SET)
    ...
    if elapsed_ms <= 0:
        ...
        let kb_per_s: Int = (total_bytes * 1000) / (elapsed_ms * 1024)
        print "  blocks  : " + str(blocks)
        print "  total   : " + str(total_mb) + " MB"
        print "  elapsed : " + str(elapsed_ms) + " ms"
        print "  randwrite: " + str(kb_per_s) + " KB/s"
